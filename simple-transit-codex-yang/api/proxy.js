/**
 * 透明反向代理：将 /v1/* 转发到 https://weiwanga.top/v1/...
 * 请求体不用 req.pipe(upstreamReq)，避免上游提前结束连接时出现 ClientRequest write after end。
 * 响应体用手动 write + writableEnded 检查，避免向已结束的 res 写入。
 */

import https from 'https';
import { URL } from 'url';

// OpenAI 协议的客户端（例如openclaw）比较“懒”，它不帮你拼接版本号 /v1，只拼接核心路由 /chat/completions。
// 所以需要在中转地址中手动把 /v1 塞进 baseUrl 里，当做“垫脚石”，骗过 Vercel 的路由拦截系统，让请求顺利进入你的代理脚本。
const UPSTREAM_ORIGIN = 'https://weiwanga.top';
...
/** 将客户端请求体写入 upstreamReq，避免 pipe 在对方已 end 时继续写 */
